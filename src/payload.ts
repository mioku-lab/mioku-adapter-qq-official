...
export type SendUnit =
  ...
  | { kind: 'media'; fileType: 1 | 2 | 3 | 4; file: string | Buffer; name?: string }
...
const fileOf = (data: Record<string, unknown>): string | Buffer | undefined => {
  ...
  const attachment = data.attachment as { url?: string; file?: string } | undefined
  if (typeof attachment?.url === 'string') return attachment.url
  if (typeof attachment?.file === 'string') return attachment.file
  return undefined
}

const buttonToOfficial = (button: ButtonOptions, index: number): OfficialButton | null => {
  ...
  return {
    ...
    action: {
      ...
      ...(action === 'command'
        ? { enter: button.enter ?? true, ...(button.reply ? { reply: true } : {}) }
        : {}),
      unsupport_tips: button.unsupportedTips ?? '当前客户端版本过低,暂不支持该操作',
    },
  }
}

const imageMarkdown = (url: string, data: Record<string, unknown>): string => {
  const summary = typeof data.summary === 'string' && data.summary ? data.summary : '图片'
...
export const keyboardOf = (buttons: OfficialButton[], appId: string): KeyboardPayload | undefined => {
 ...
 */
export const buildSendUnits = (message: MessageInput, options: BuildUnitsOptions): BuiltUnits => {
  ...
  const flush = (): void => {
    if (text.trim()) units.push({ kind: 'markdown', content: text, buttons: [] })
    text = ''
  }
  ...
  const pushMedia = (segType: string, data: Record<string, unknown>): void => {
    ...
    flush()
    const fileType = segType === 'image' ? 1 : segType === 'video' ? 2 : segType === 'record' ? 3 : 4
    ...
    for (const seg of asMessage(input)) {
      ...
      switch (seg.type) {
        ...
        case 'image': {
          ...
          if (imageMode === 'markdown' && url) {
            text += imageMarkdown(url, data)
          } else {
        ...
        case 'file':
          ...
          break
        case 'reply':
          if (!refMessageId && typeof data.message_id === 'string') refMessageId = data.message_id
          break
        case 'button':
          if (typeof data.label === 'string') buttons.push(data as unknown as ButtonOptions)
          break
        case 'node': {
          flush()
          const content = data.content
          if (Array.isArray(content)) for (const item of content) walk(item as MessageInput)
          break
        }
        case 'face':
        case 'json':
...
export const shouldDegrade = (err: unknown): boolean => {
  ...
  return ['50037', '50056', '304036', '304061', '40034011', '40034008', '40034009'].includes(
    String(code),
  )
}
