import { asMessage, atOf, buildRoutes, createFriendRef, createGroupRef } from 'mioku'
import { MessageSegmentImpl, createMessage } from 'mioku'

import { buildMessage, parseSceneExt, senderOf, userIdOf } from './segments'
...
export type MessageType = 'GROUP_AT_MESSAGE_CREATE' | 'GROUP_MESSAGE_CREATE' | 'C2C_MESSAGE_CREATE'
...
const identityOf = (
  params: EventBuildParams & { eventType: string; messageId?: string; time?: number },
) => ({
...
const replyOf = (
  ...
  return async (input, options) => {
    ...
    if (opts?.quote && params.eventMessageId) {
      content = [new MessageSegmentImpl('reply', { message_id: params.eventMessageId }), ...asMessage(input)]
    }
    const target =
...
export const buildQQMessageEvent = (
  ...
  const conversation = { type: isGroup ? ('group' as const) : ('private' as const), id: isGroup ? groupId! : userId }
  ...
  const isToMe =
    ...
    (d.mentions ?? []).some((m) => (m.member_openid ?? m.id) === bot.bot_id)
  ...
  return {
    ...
    identity: identityOf({ ...buildParams, eventType: isGroup ? 'message.group' : 'message.private', messageId, time }),
    self_id: bot.bot_id,
    bot,
    time,
    raw: d,
    ...
    recall: async () => {
      await client.delete(
        isGroup ? `/v2/groups/${groupId}/messages/${messageId}` : `/v2/users/${userId}/messages/${messageId}`,
      )
    },
  }
}
...
export const buildInteractionEvent = (
  ...
  const segments: MessageSegment[] = [new MessageSegmentImpl('text', { text: buttonData })]
  ...
  return {
    kind: 'message',
    type: 'message',
    routes: buildRoutes(adapterName, 'message', isGroup ? 'group' : 'private', 'callback'),
    identity: identityOf({ ...buildParams, eventType: 'interaction.callback', messageId: eventId, time }),
    self_id: bot.bot_id,
    bot,
    time,
    raw: d,
...
export const buildQQNoticeEvent = (
  ...
  return {
    kind: 'notice',
    type: 'notice',
    routes: buildRoutes(adapterName, 'notice', mapping.noticeType, mapping.subType),
    identity: {
      adapter: adapterName,
      bot_id: bot.bot_id,
    ...
    group_id: isGroup && typeof d.group_openid === 'string' ? d.group_openid : undefined,
  }
}
