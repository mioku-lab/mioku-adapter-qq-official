...
import { API_BASE, DEFAULT_INSTANCE, DEFAULT_INTENTS, SANDBOX_API_BASE, normalizeInstances } from './config'
...
import type { QQOfficialAdapterConfig, QQOfficialInstanceConfig } from './config'

const ADAPTER_NAME = 'qq-official'

const buildAdapter = (instance: QQOfficialInstanceConfig, logger: Logger): Adapter => {
  const resolved = { ...DEFAULT_INSTANCE, ...instance }
  const apiBase = instance.apiBase ?? (resolved.sandbox ? SANDBOX_API_BASE : API_BASE)
  ...
  const eventParams = () => {
    ...
    return { adapterName: ADAPTER_NAME, bot, client, passive, refs, logger }
  }

  const ensureBot = async (user?: { id?: string; username?: string }): Promise<void> => {
    ...
    if (!unregisterStatus) {
      unregisterStatus = registerStatusProvider(
        ...
        (): AdapterStatus => ({
          ...
          impl: 'QQ OpenAPI v2',
          protocol: 'v2',
          stats: { sent: sendCount, received: receiveCount, groups: groups.size },
          data: { appId: instance.appId, sandbox: Boolean(resolved.sandbox), imageMode: resolved.imageMode },
        }),
      )
    }
  ...
  const registerCapabilities = (ctx: AdapterContext, currentBot: QQOfficialBot): void => {
    ...
    unregisterCapabilities = [
      ...
      ctx.registerCapability(profileSet, target, (req) => currentBot.setProfile(req)),
  ...
  const handleInteraction = async (d: QQInteractionPayload): Promise<void> => {
    ...
    // 3 秒超时,先回应再派发
    client.request('PUT', `/interactions/${interactionId}`, { code: 0 }).catch((err) => {
  ...
  const handleDispatch = async (dispatch: {
    ...
    if (t === 'READY') {
      const user = (d as { user?: { id?: string; username?: string } } | undefined)?.user
    ...
    if (t === 'C2C_MESSAGE_CREATE' || t === 'GROUP_AT_MESSAGE_CREATE' || t === 'GROUP_MESSAGE_CREATE') {
      ...
      if (payload.group_openid) groups.set(String(payload.group_openid), { group_id: String(payload.group_openid) })
      receiveCount += 1
      const event = buildQQMessageEvent({ ...params, eventType: t as MessageType, d: payload })
      if (!event) {
        logger.debug(`消息事件解析失败 ${t}#${id ?? ''}`)
        return
      }
      await context.dispatch(event)
      return
    ...
    if (mapping) {
      ...
      await context.dispatch(
        buildQQNoticeEvent({ ...params, eventType: t, d: d as Record<string, unknown>, mapping }),
      )
      return
    }
  ...
  return {
    ...
    async start(ctx: AdapterContext): Promise<void> {
      ...
      token = new TokenManager(driver, instance.appId, instance.appSecret, logger)
      client = new QQClient(driver, apiBase, token, logger)
      uploader = new MediaUploader(client, logger)
      passive = new PassiveManager(resolved.passiveWindowMs, resolved.maxPassiveReplies)
        ...
        },
        {
          ...
          onDisconnect: async () => {
            if (bot && botData.online) {
              ...
              await context?.emitLifecycle({ type: 'bot:disconnected', bot, reason: 'connection closed' })
            }
          },
        },
      )
    ...
    async stop(reason?: string): Promise<void> {
      if (bot && botData.online) {
        botData.online = false
        try {
          await context?.emitLifecycle({ type: 'bot:disconnected', bot, reason: reason ?? 'stop' })
        } catch (err) {
          logger.warn(`emit bot:disconnected 失败: ${err instanceof Error ? err.message : String(err)}`)
        }
      }
      if (gateway) {
...
export const qqOfficialAdapterDefinition = defineAdapter<QQOfficialAdapterConfig>({
  name: ADAPTER_NAME,
  version: '1.0.0',
  apiVersion: 1,
  validateConfig: (config): QQOfficialAdapterConfig => {
    ...
    if (instances.length === 0) {
      throw new Error('qq-official.instances must contain at least one instance')
    }
    for (const [index, instance] of instances.entries()) {
      ...
      if (!instance.appSecret || typeof instance.appSecret !== 'string') {
        throw new Error(`qq-official.instances[${index}].appSecret is required`)
      }
    }
    return { instances }
  },
  create: (options: AdapterFactoryOptions<QQOfficialAdapterConfig>): Adapter => {
    const instances = options.config.instances
    const adapters = instances.map((instance) => buildAdapter(instance, options.logger))
    return {
      name: ADAPTER_NAME,
      version: '1.0.0',
